...
compute_elbo <- function(X, qg){
  ...
  term2 = ll_l - sum(sum(qf$mean)*ql$mean + rowSums(X)*ql$mean_log)
  ## compute Eq(log(gF(f)/qF(f)))
  term3 = ll_f - sum(sum(ql$mean)*qf$mean + colSums(X)*qf$mean_log)
...
```{r}
start =  proc.time()
out_ebpmf = ebpmf_rank1_exponential(sim$X, init,maxiter = 1)
