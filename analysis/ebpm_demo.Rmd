...
df = data.frame(n = 1:length(x), x = log10(x+1), lam_true = log10(lam_true+1), lam_hat = log10(fit$pm + 1))

ggplot(df)  + geom_point(aes(x = n, y = lam_true, color = "true"), cex = 0.5) +
               geom_point(aes(x = n, y = x, color = "x"), cex = 0.5) +
               geom_point(aes(x = n, y = lam_hat, color = "lam_hat"), cex = 0.5) +
