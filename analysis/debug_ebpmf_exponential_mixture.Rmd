...
I managed to find the bug in `ebpmf_exponential_mixture`. Now we can make `ELBO` increase monotonically if we fix the scale/grid. But when we unfix the grid, the `ELBO` does  not increase monotonically. 
...
rm(out)
## rank-k: fix nothing
out =  ebpmf::ebpmf_exponential_mixture(X = sim$X, K = K, maxiter.out = maxiter, verbose = F)
