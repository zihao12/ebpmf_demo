...
* Numerical issue when computing `L`: at iteration 107, I get ` Error in verify.likelihood.matrix(L) : Input argument "L" should be a numeric matrix with >= 2 columns, >= 1 rows, all its entries should be non-negative, finite and not NA, and some entries should be positive`. 
...
```{r}
## ebpmf
out = ebpmf::ebpmf_exponential_mixture_experiment(sim$X, K, maxiter.out = 106, verbose = F)
elbo_ebpmf= out$ELBO
out_ebpmf = out$qg
