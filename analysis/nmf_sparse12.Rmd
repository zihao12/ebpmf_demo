...
I compare `ebpmf` methods with different prior families: `point-gamma`, `invBeta-gamma`, `gammamix`
...
## compare runtime
```{r}
t(list(lee = fit_lee$run.time[[3]], pg = fit_ebpmf_pg$runtime, bg = fit_ebpmf_bg$runtime, gammamix = fit_ebpmf_gammamix$runtime))
```
...
rmse = c(0, RMSE(lambda, lam_mle), RMSE(lambda, lam_ebpmf_pg), 
         RMSE(lambda, lam_ebpmf_bg), RMSE(lambda, lam_ebpmf_gammamix))
js = c(0, JS(lambda, lam_mle), JS(lambda, lam_ebpmf_pg), 
         JS(lambda, lam_ebpmf_bg), JS(lambda, lam_ebpmf_gammamix))
kl = c(0, KL(lambda, lam_mle), KL(lambda, lam_ebpmf_pg), 
...
dist = data.frame(cbind(rmse, js, kl, ll), row.names = c("oracle","mle", "point-gamma", "invBeta-gamma", "gammamix"))
dist
```
