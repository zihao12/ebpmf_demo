...
fit_ebpmf_two_gamma = ebpmf.alpha::ebpmf_two_gamma(X = t(X), K = k, maxiter.out = 1000, verbose = FALSE)
```
...
## Conclusion
* `ebpmf` with two gammas has optimization issues. The default initialization for `ebpm_two_gamma` is using k-means; however, when applying this, the ELBO does not increase monotonically, and get NaN in `nlm`. So I initialize `ebpm_two_gamma` with fixed `g` each time. But the estimated $\hat{g}$ for each loading & factor has $\pi_0 = 0$, so only one gamma is used. Why is that ??

* ELBO: Truth > ebpmf >  MLE

* By comparing $A$, we can't find obvious differences between MLE and ebpmf result. 
