...
#plot(l_1[min_idx,]/(x_l[min_idx,]*s_l), ylab  = "posterior mean/MLE") ## MLE is 0 after a few iterations
```
...
print((exp(l_log_1[large_idx,idx_])/(x_l[large_idx,idx_]/s_l[idx_]))[990:1000])

plot(exp(l_log_1[max_idx,idx_])/(x_l[max_idx,idx_]/s_l[idx_]), ylab  = "exp(posterior mean log)/MLE")
print((exp(l_log_1[max_idx,idx_])/(x_l[max_idx,idx_]/s_l[idx_]))[990:1000])

## x/s becomes 0 afterwards
plot(exp(l_log_1[min_idx,idx_])/(x_l[min_idx,idx_]/s_l[idx_]), ylab  = "exp(posterior mean log)/MLE")
```
...
plot(x_l[,1000])
print(s_l[1000])
fit_l = ebpm_gamma_mixture_single_scale(x = x_l[,1000], s = s_l[1000])

top_idx = sort(fit_l$fitted_g$pi, index.return =  TRUE, decreasing = TRUE)$ix[1:3]

## biggest proportions
fit_l$fitted_g$pi[top_idx]

## shapes for the biggest proportions
fit_l$fitted_g$shape[top_idx]

## scales for the biggest proportions
fit_l$fitted_g$scale[top_idx]

## look  at the proportions of 0s
print(sum(x_l[,1000] == 0)/n)

## look  at the proportions of nozero (nonzero in true L as well)
print(sum(x_l[,1000] > 100)/n)

## look  at the proportions of zeros (zero in true L)
print(sum(x_l[,1000] > 0 & x_l[,1000] < 100)/n)
```
So basically is one very small shape (0.3 weight), and two larger shapes (they are neighboring grids) for the rest of the weight
