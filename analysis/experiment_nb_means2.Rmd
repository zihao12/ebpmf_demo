...
exper_nb_means <- function(K = 3, p = 999, maxiter = 10, seed = 123, verbose =  FALSE){
  ...
  mask_small = Phi < 0.9
  Phi[mask_small] = 1e-4
  #id_tmp = rbinom(sum(!mask_small),1, 0.2)
  Phi[!mask_small] = 1e+4
...
There is optimization issue when initializing from truth ...

## comparee loglikelihood
```{r}
plot(exper$fit_init_random2$progress, col = "red", xlab  = "niter", ylab = "loglikelihood", type = "l", ylim = c(min(exper$fit_init_random2$progress), max(c(exper$fit_init_random2$progress, exper$ll_oracle))))
...
plot(exper$Phi, Phi_1, log = "y", xlab = "phi_true", ylab = "phi_fit")
```
