...
L[1:n/3,1] = signal_level
F[1:p/3,1] = signal_level
L[,2] = signal_level + noise_level*runif(n)
F[,2] = signal_level + noise_level*runif(p)
Z1 = outer(L[,1], F[,1])
Z2 = outer(L[,2], F[,2])
image(t(Z1))
image(t(Z2))
lambda = Z1 + Z2
X = matrix(rpois(n = n*p, lambda = lambda), nrow = n)
image(t(X))
