...
* Other technical issue: the computation can be very slow in some initialization (for example, from the truth, which  is  very  sparse), whereas it is fast if both L,F are initialized from uniform from 0 to 1. This is because sometimes we choose too many grids: for example the `ebpm` problem  has `x` from `1e-40` to `1e+10` and we choose `m = 2^0.25`, then we end up choosing almost 300 grids. 
