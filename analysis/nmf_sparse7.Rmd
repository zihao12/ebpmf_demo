...
* Compared with MLE result, the shrinkage effect is not very obvious,and that `ebpmf` chooses to shrink noise (look at factors/loadings 4).  
...
## Model fitting
* I initialize using `lee`'s multiplicative update for 100 iterations. 

* Then I run `ebpmf_gamma_mixture` with `scale = 1`, `shape` chosen adaptively (so that prior mean cover $x/s$). I run  it  only a few iterations. (The range of $x/s$ is huge, so there are too many grids and the algorithm is  slow). 
```{r}
set.seed(123)
...
Compare $L,F$ (after transforming into multinomial model): initialization vs fitted
```{r}
par(mfrow=c(2,k))
for(idx in 1:k){
...
Compare $\zeta$: initialization vs fitted
```{r}
zeta_em = ebpmf::get_Ez(X = X, qg = initialize_qg_from_LF(L0 = fit_em_random$W, F0 = t(fit_em_random$H)), K = k)$zeta
zeta_gamma_mix = ebpmf::get_Ez(X = X, qg = fit_gamma_mix_random$qg, K = k)$zeta
...
Take a look at the $\hat{g}$ for L, F. 
```{r}
par(mfrow=c(2,2))
for(idx in 1:k){
...
```{r}
zeta_gamma_mix_uniform = ebpmf::get_Ez(X = X, qg = fit_gamma_mix_uniform_weight$qg, K = k)$zeta
