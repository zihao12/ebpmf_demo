...
## Goal 
Use data where `ebpm_point_gamma` fails and see how `ebpm_beta_gamma` does. 

I start from the previous experiment: https://zihao12.github.io/ebpmf_demo/demo_ebpm_beta_gamma.html

## Summary
With  proper initialization (I  basically cheated here) `ebpm_beta_gamma` does well. 
...
## Small experiment (x is no longer integer)
```{r}
set.seed(123)
n = 99
lam = replicate(n, 0)
...
(fixed `g` close to best estimate from `ebpm_point_gamma` in previous analysis)
```{r}
